...
def add_vehicle(request):
    ...
    if request.method == "POST":
        ...
        try:
            Vehicle.objects.create(parkingnumber=pn, category=category, vehiclecompany=vc, regno=rn, ownername=on,
                                   ownercontact=oc, pdate=pd, intime=it, outtime='', parkingcharge='', remark='', status=status)
            error = "no"
        except:
            error = "yes"
...
def betweendate_report(request):
    ...
    if request.method == "POST":
        ...
        vehiclecount = Vehicle.objects.filter(
            Q(pdate__gte=fd) & Q(pdate__lte=td)).count()
        d = {'vehicle': vehicle, 'fd': fd,
             'td': td, 'vehiclecount': vehiclecount}
